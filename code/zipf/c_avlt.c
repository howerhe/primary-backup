...
static inline int compare(double _node_key, double _input_key) {
...
static inline void _balance_adjust_after_insertion_turn(int _rotation_type, _avlt_node_t *_top, 
...
static inline void _balance_adjust_after_deletion_turn(int _rotation_type, _avlt_node_t *_top, 
				       _avlt_node_t *_middle, _avlt_node_t *_bottom) {
  switch (_rotation_type) {
  case _RR:
...
static inline void _rr_rotation(avlt_t *_avlt, 
			       _avlt_node_t *_top, _avlt_node_t *_middle) {
...
static inline void _ll_rotation(avlt_t *_avlt, 
			       _avlt_node_t *_top, _avlt_node_t *_middle) {
  _avlt_node_t *_parent = _top->_parent;
...
static inline void _balance_after_insert(avlt_t *_avlt, 
					_avlt_node_t *_node, _avlt_node_t *_new_node) {
  ...
  while (curr != NULL) {
    ...
    if (curr->_balance == _LEFT2X) {
      assert(prev->_balance == _LEFT || prev->_balance == _RIGHT);
      if (prev->_balance == _LEFT) {
	_rr_rotation(_avlt, curr, prev);
	_balance_adjust_after_insertion_turn(_RR, curr, prev, NULL);
	break;
      } 
      else {
	_lr_rotation(_avlt, curr, prev, prevprev);
	_balance_adjust_after_insertion_turn(_LR, curr, prev, prevprev);
	break;
      }
    } 
	...
	_balance_adjust_after_insertion_turn(_LL, curr, prev, NULL);
	break;
      } 
      else {
	_rl_rotation(_avlt, curr, prev, prevprev);
	_balance_adjust_after_insertion_turn(_RL, curr, prev, prevprev);
	break;
      }
    } 
    prevprev = prev;
...
static inline void _balance_after_delete(avlt_t *_avlt, _avlt_node_t *_node) {
  ...
  // if _node->_balance == 0 then this means the height has decreased
  ...
  while (_top != NULL) {
	...
	_balance_adjust_after_deletion_turn(_LR, _top, _middle, _bottom);
      } 
      else {
	_rr_rotation(_avlt, _top, _middle);
	_balance_adjust_after_deletion_turn(_RR, _top, _middle, NULL);
      }
    } 
      ...
      if (_middle->_balance == _LEFT) {
      	...
      	_balance_adjust_after_deletion_turn(_RL, _top, _middle, _bottom);
      } 
      else {
	_ll_rotation(_avlt, _top, _middle);
	_balance_adjust_after_deletion_turn(_LL, _top, _middle, NULL);
      }
    } 
    else if (_top->_balance == 0) {
      if (_top->_parent != NULL) {
	if (_top == _top->_parent->_left_child) {
	  _top->_parent->_balance = _top->_parent->_balance - _LEFT;
	} 
	else if (_top == _top->_parent->_right_child) {
	  _top->_parent->_balance = _top->_parent->_balance - _RIGHT;
	} 
	else {
	  assert(false);
	}
...
static inline _avlt_node_t * _map_search(avlt_t *_avlt, double _key) {
...
static inline _avlt_node_t * _map_search_closest(avlt_t *_avlt, double _key) {
  double diff = _INIT_VAL;
  double tmp_diff = 0;
  _avlt_node_t *_closest = NULL;
  _avlt_node_t *_curr = _avlt->_root;
  while (_curr != NULL) {
    int _cmp_result = compare(_curr->_key, _key);
    tmp_diff = _curr->_key - _key;
...
int avlt_validate_order(_avlt_node_t *_node) {
  ...
  if (_left_key >= _node->_key) {
    fprintf(stderr, "### AVLT VALIDATION FAILED ### left %f node %f\n", _left_key, _node->_key);
    return 0;
  } 
  else if (_right_key <= _node->_key) {
    fprintf(stderr, "### AVLT VALIDATION FAILED ### node %f right %f\n", _node->_key, _right_key);
    return 0;
  } 
  else {
    return _node->_key;
...
void * avlt_search(avlt_t *_avlt, double _key) {
...
_avlt_node_t * avlt_search_closest(avlt_t *_avlt, double _key) {
  _avlt_node_t * _found = _map_search_closest(_avlt, _key);
  if (_avlt->_size > 0)
	  assert(_found != NULL);
  return _found;
}
...
void avlt_insert(avlt_t *_avlt, double _key, void *_value) {
  int _cmp_result;
  _avlt_node_t *_curr = _avlt->_root;
  ...
  while (_curr != NULL) {
	...
	break;
      ...
      fprintf(stderr, "ERROR: Redundant key (%f) value found while inserting!\n", _key);
      assert(false);
    }
  }  // end of while (curr != NULL) {
...
void avlt_delete(avlt_t *_avlt, double _key) {
  ...
  _avlt_node_t *_replacement_node = _get_max_of_left_subtree(_avlt_node_to_delete);
  if (_replacement_node == NULL) {
    _replacement_node = _get_min_of_right_subtree(_avlt_node_to_delete);
  }
...
static void print_node(_avlt_node_t *_node, int _level, int _print_level) {
  ...
  if (_level == _print_level) {
    fprintf(stderr, "%lld(%d|%d)", (long long int) _node->_key, _level, _node->_balance);
  }
  if (_node->_right_child != NULL) {
...
void print_all_node(_avlt_node_t *_node, int _level) {
  if (_node->_left_child != NULL) {
    print_all_node(_node->_left_child, _level + 1);
  } 

  fprintf(stderr, "%lld(%d - %d) ", (long long int) _node->_key, _level, _node->_balance);

  if (_node->_right_child != NULL) {
    print_all_node(_node->_right_child, _level + 1);
  }
}
